...
power :: (Fractional a, Eq a) => a -> Int -> (a,Int)
power _ 0 = (1,0)
power 0 y 
    | y < 0 = undefined
    | y > 0 = (0,y)
power x y 
    | y > 0 = (x * fst (power x (y-1)), y)
    | y < 0 = ((1/x) * fst (power x (y+1)), -y)
...
fib :: (Num a, Eq a) => Int -> [a]
fib n 
    | n <= 0 = [0]
    | n > 0 = reverse (take (n+1) (0 : 1 : genFib [0, 1]))
    where
...
piCalc :: (Fractional a, Integral b, Ord a) => a -> (a, b)
piCalc t = piCalc' 1 0 t 0

piCalc' :: (Ord a, Fractional a, Integral b) => a -> a -> a -> b -> (a, b)
piCalc' d p t i
    | abs (4/d) <= t    = (p,i)
    | otherwise         = piCalc' d' p' t (i+1)
    where
        d' = stepReverseSign d 2
        p' = p + (4/d)
